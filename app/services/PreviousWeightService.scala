...
class PreviousWeightService @Inject()(polybodyConnector: PolybodyConnector)(implicit ec: ExecutionContext) {
  ...
  def getPreviousWeights(username: String): Future[Either[CustomErrorHandler, List[PreviousWeight]]] = {
    val data: Future[Either[CustomErrorHandler, ArrayBuffer[Value]]] = polybodyConnector.getPreviousWeights(username)

...
class $model;format="Camel"$ControllerSpec extends PlaySpec with GuiceOneAppPerTest with Injecting {
  ...
  "$model;format="Camel"$Controller GET" should {
    ...
    "render the LandingPage page from a new instance of controller" in {
    ...
    "render the LandingPage page from the application" in {
    ...
    "render the LandingPage page from the router" in {
    ...
    "process form" in {
      val request = {
        FakeRequest(POST, "/$model;format="camel"$")
          .withFormUrlEncodedBody("name" -> "play", "Page2Age" -> "4")
      }
      val home = route(app, request).get
